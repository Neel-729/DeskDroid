// DeskDroid v1.3
// A smart desk clock with timer, stopwatch and reminders. Many more features planned for future updates.
// there are still some rough edges and unpolished features, but the core functionality is all there and working well.
// The code is provided as-is, with no warranty or guarantee of any kind. Use at your own risk.
// The project is open-source and available on GitHub:
// There are further improvements and features planned, but I wanted to share the code in its current state for anyone interested in building their own or learning from it.
// This is final version of the code for the initial release, but I may update it in the future with bug fixes, optimizations and new features based on user feedback and my own ideas.
...
void loadQuote(){
  ...
  for(int i=0;i<16;i++){
    if(activeQuoteLength+i<sizeof(activeQuote)-1)
...
void setup(){
  ...
  timerTotalMillis=((unsigned long)timerHours*3600UL+
                    (unsigned long)timerMinutes*60UL+
                    (unsigned long)timerSeconds)*1000UL;
      ...
      case TIMER:
        ...
        if(timerAlarmActive){
          ...
          if(timerEditField==EDIT_HOURS)
            ...
            timerEditField=EDIT_SECONDS;
          else
            timerEditField=EDIT_HOURS;
        }

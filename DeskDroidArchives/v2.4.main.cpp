...
const uint8_t SETTINGS_COUNT=9;
  ...
  if(!settingsMenuActive){
    ...
    switch(settingsIndex){
      ...
      writeRow(0,"LED Brightness");
     {
      ...
      break;
      case 3: writeRow(0,"Buzzer"); writeRow(1,deviceSettings.buzzer?"ON":"OFF"); break;
      case 4: writeRow(0,"Quotes"); writeRow(1,deviceSettings.quotes?"ON":"OFF"); break;
      case 5: writeRow(0,"Time Format"); writeRow(1,deviceSettings.format24?"24H":"12H"); break;
      case 6:{
      ...
      case 8: writeRow(0,"DeskDroid"); writeRow(1,"v2.3"); break;
    }
  }
}
...
void initHardware(){
  ...
  pixels.begin();
  pixels.setBrightness(deviceSettings.ledBrightness * 25.5); // scale (0–5 → 0–250)
  pixels.clear();
...
case LED_IDLE:
  ...
  switch(idlePreset){
    ...
    case IDLE_BREATH: {
      ...
      brightness += dir;
      if(brightness <= 0 || brightness >= 255) dir = -dir;

      uint32_t color = pixels.Color(brightness/4, 0, brightness/6);
      pixels.fill(color);
      break;
    }
    ...
    case IDLE_PULSE: {
      if(toggle) pixels.fill(pixels.Color(30,0,20));
      else pixels.clear();
      break;
    }
  }
...
void loop(){
  ...
  if(ev==EVENT_ROTATE_CW || ev==EVENT_ROTATE_CCW){
    ...
    if(currentState==TIMER && timerEditMode && !timerRunning){
      ...
      if(settingsIndex==5 && settingsEditing && timeAdjustMode){
        if(adjustHourField){ adjustHour=(adjustHour+step+24)%24; }
        else { adjustMinute=(adjustMinute+step+60)%60; }
      }
      else if(settingsIndex==6 && settingsEditing && dateAdjustMode){
        ...
        pixels.setBrightness(deviceSettings.ledBrightness * 25.5); // scale (0–10 → 0–255)
        pixels.show();
        }

      else if(settingsIndex==3){ deviceSettings.buzzer=!deviceSettings.buzzer; }
      else if(settingsIndex==4){ deviceSettings.quotes=!deviceSettings.quotes; }
      else if(settingsIndex==5){ deviceSettings.format24=!deviceSettings.format24; }
      stateChanged = true;
    }
    else if(currentState==SETTINGS && settingsMenuActive && !settingsEditing){
